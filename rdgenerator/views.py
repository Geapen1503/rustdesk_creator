    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if permissionsDorO == "default":
                ...
                decodedCustom['override-settings'][
                    'allow-remote-config-modification'] = 'Y' if enableRemoteModi else 'N'
        ...
        if version == '1.3.2' or version == '1.3.1' or version == '1.3.0':
            if platform == 'windows':
                ...
                url = 'https://api.github.com/repos/' + _settings.GHUSER + '/creator/actions/workflows/pre133-generator-macos.yml/dispatches'
            else:
                url = 'https://api.github.com/repos/' + _settings.GHUSER + '/creator/actions/workflows/pre133-generator-windows.yml/dispatches'
        ...
        if response.status_code == 204:
            ...
            return JsonResponse({"error": "Something went wrong", "details": response.text})
...
def check_for_file(request):
    ...
    if status == "Success":
        ...
        return render(request, 'waiting.html',
                      {'filename': filename, 'uuid': uuid, 'status': status, 'platform': platform})
...
def startgh(request):
    ...
    ####from here run the github action, we need user, repo, access token.
    url = 'https://api.github.com/repos/' + _settings.GHUSER + '/rdgen/actions/workflows/generator-' + data_.get(
        'platform') + '.yml/dispatches'
    data = {
        "ref": "master",
        "inputs": {
